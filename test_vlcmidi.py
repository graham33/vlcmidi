...
_TEST_MIDI_MSG = [
    (_MIDI_CONTROL_CHANGE_MESSAGE_TYPE << 4)
    | _TEST_CHANNEL,  # Control Change message
    _TEST_CONTROLLER,
...
@pytest.mark.skipif(not _rtmidi_virtual_ports_available(),
                    reason="rtmidi virtual ports not available")
@pytest.mark.timeout(5)
def test_midi_virtual():
    midi_out, out_port_name = rtmidi.midiutil.open_midioutput(
        use_virtual=True, port_name="vlcmidi test_midi_virtual")
...
def test_vlc_status(requests_mock):
    requests_mock.get(
        'http://localhost:8080/requests/status.json?command=pl_play',
        json={"state": "playing"})
    # TODO: URL
